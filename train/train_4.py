...
class MyEmbeddingDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        audio_emb = entry['audio_emb']  # [Ta, 1024]
        text_emb  = entry['text_emb']   # [Tt, 768]
...
class DeepMLP(nn.Module):
    ...
    def forward(self, audio_vec, text_vec):
        x = torch.cat([audio_vec, text_vec], dim=1)  # [B, 1536]
        out = self.mlp(x)                            # [B, num_classes]
        return out


def main(batch_size=8, num_epochs=10, lr=1e-3):
    set_seed(42)
    ...
    # ============ former_model.parameters() + classifier.parameters() ============
    ...
    for epoch in range(num_epochs):
        ...
        for padded_audios, audio_masks, padded_texts, text_masks, labels in pbar:
            ...
            audio_vec = out_dict["audio_all"]  # [B,768]
            text_vec  = out_dict["text_all"]   # [B,768]

            logits = classifier(audio_vec, text_vec)  # [B, num_classes]
        ...
        print(f" [*] Saved latest model checkpoint at epoch {epoch+1} -> stmil_latest.pth")
        ...
        if test_acc > best_test_acc:
            ...
            print(f" [*] Best model so far (test_acc={best_test_acc:.4f}) -> stmil_best.pth")
...
if __name__ == "__main__":
    main(batch_size=16, num_epochs=50, lr=1.5e-5)
